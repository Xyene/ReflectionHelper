...
public class Reflection {
    ...
    public static <T> FieldContainer<T> declaredField(String name) {
        return new FieldContainer<T>(name, true);
    }

    public static <T> FieldContainer<T> field(String name) {
        return new FieldContainer<T>(name, false);
    }
    ...
    public static class FieldContainer<T> extends Container {
        ...
        public <T> FieldContainer<T> ofType(Class<T> clazz) {
            FieldContainer<T> clone = new FieldContainer<T>(name, declared);
            clone.target = target;
            clone.in = in;
            return clone;
        }
        ...
        public Field getRaw() {
            try {
                Field raw = declared ? target.getDeclaredField(name) : target.getField(name);
                access(raw);
                return raw;
            } catch (Exception e) {
                throw new RuntimeException(e);
